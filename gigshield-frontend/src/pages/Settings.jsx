      ...
      <div>
        <div className="page-header">
          <h1>
            ...
            <span className="admin-badge-title" style={{ fontSize: '0.6rem', padding: '4px 8px', borderRadius: '4px', background: 'var(--accent-amber)', color: '#0f172a', marginLeft: '12px', verticalAlign: 'middle', textTransform: 'uppercase', fontWeight: 800 }}>Admin</span>
          </h1>
            ...
            <div className="settings-card">
              ...
              <form onSubmit={handlePasswordChange} style={{ padding: '0 16px 16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '4px' }}>
                  Update the master administrator password.
                </div>
                ...
                <button type="submit" disabled={passwordLoading} className={`btn btn-primary ${passwordLoading ? 'btn-loading' : ''}`} style={{ marginTop: '4px' }}>
          ...
          {[
            ...
            <div key={platform.name} className="settings-field" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {platform.status === 'Connected' ? (
                 ...
                 <button className="btn btn-secondary btn-sm" onClick={() => showSuccess('OAuth Initiated', `Connecting to ${platform.name} secure API...`)}>Connect</button>
              )}
            </div>
          ))}
        </div>
